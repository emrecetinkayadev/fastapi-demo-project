from blog.users.models import User, UserCreate, UserUpdate, UserRead, UserDB
import typing as t
from sqlalchemy.orm import Session


def get_all(db: Session) -> t.List[t.Optional[UserRead]]:
    user_table = db.query(UserDB).all()
    return user_table
...
def update(user_id: int, update_data: UserUpdate, db: Session) -> t.Optional[User]:
    ...
    user_query = db.query(UserDB).filter(UserDB.id == user_id)
...
def check_email(user_mail, db: Session) -> bool:
    user = db.query(UserDB).filter(UserDB.email == user_mail).first()
    if not user:
        return False
    return True
